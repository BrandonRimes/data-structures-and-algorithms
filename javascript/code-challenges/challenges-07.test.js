...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= arr.length; i++) {
    result.push(arr.slice(i, arr.length).join(''));
  }
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let ing = recipe.ingredients.slice(i,i+1);
    let item = (ing) => ing.indexOf(' ', 4);
    let start = item(...ing)+1;
    let str = (ing) => ing.slice(start);
    result.push(str(...ing));
  }
  return result;
};
